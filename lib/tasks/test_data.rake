namespace :test_data do
  ...
  task populate: :environment do
    ...
    puts "Seeding database with #{PLAYER_COUNT} players, #{STATS_PER_PLAYER} stats per player, and associated weapons, pickups, and medals..."
    ...
    Player.find_each(batch_size: BATCH_SIZE) do |player|
      STATS_PER_PLAYER.times do
        # Generate a random created_at timestamp within the last 5 years
        created_at = Faker::Time.between(from: 5.years.ago, to: Time.current)
        ...
        pickups << PICKUP_FIELDS.index_with { Faker::Number.between(from: 0, to: 20) }
          .merge({ stat_id: stats.size, created_at: created_at, updated_at: created_at })
